...
namespace FridgeManagementSystem.Areas.CustomerManagementSubSystem.Controllers
{
    ...
    public class InventoryLiaisonController : Controller
    {
        ...
        public async Task<IActionResult> Receive(int? fridgeId, int? purchaseRequestId)
        {
            ...
            {
                model.SerialNumber = SerialNumberGenerator.GenerateFridgeSerialNumber(
                    model.Brand, model.Type, model.FridgeId);
            }
        ...
        public async Task<IActionResult> Receive(ReceiveFridgeVm model)
        {
            ...
            if (model.FridgeId > 0 && (string.IsNullOrEmpty(model.Brand) || string.IsNullOrEmpty(model.ModelName) || string.IsNullOrEmpty(model.Type)))
            {
                var existingFridge = await _context.Fridge.FindAsync(model.FridgeId);
                if (existingFridge != null)
                {
                    model.Brand = existingFridge.Brand;
                    model.ModelName = existingFridge.Model;
                    model.Type = existingFridge.FridgeType;
                }
            }

            // 🔥 ALSO: If we have PurchaseRequestID but empty fields, try to get from purchase request
            if (model.PurchaseRequestID.HasValue && (string.IsNullOrEmpty(model.Brand) || string.IsNullOrEmpty(model.ModelName)))
            {
                var purchaseRequest = await _context.PurchaseRequests
                ...
                if (purchaseRequest?.Fridge != null)
                {
                    ...
                    model.ModelName = purchaseRequest.Fridge.Model;
            ...
            // 🔥 AUTO-GENERATE SERIAL NUMBER if empty USING UTILITY CLASS
            if (string.IsNullOrEmpty(model.SerialNumber))
            {
                model.SerialNumber = SerialNumberGenerator.GenerateFridgeSerialNumber(
                    model.Brand, model.Type, model.FridgeId);
            ...
            try
            {
                ...
                if (fridge == null && model.FridgeId > 0)
                {
                    ...
                    fridge = await _context.Fridge
                        .FirstOrDefaultAsync(f => f.Brand == model.Brand && f.Model == model.ModelName && f.FridgeType == model.Type);
                }
                ...
                else
                {
                    ...
                    // Only update serial number if provided and different
        ...
        public async Task<IActionResult> CreatePurchaseRequest(CreatePurchaseRequestViewModel model)
        {
            ...
            // Generate unique Request Number USING UTILITY CLASS
            var year = DateTime.Now.Year;
...
//            if (string.IsNullOrEmpty(model.SerialNumber))
//            {
//                model.SerialNumber = GenerateSerialNumber(model.Brand, model.Type, model.FridgeId);
//            }
